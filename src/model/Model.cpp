...
void ModelSpec::deserialize(BinaryFile* fb) {
  ...
  if (bHasThumb) {
    ...
    if (false) {
      string_t cached = Gu::getPackage()->makeAssetPath("cache", getName() + "-thumb-loaded.png");
...
void ModelSpec::serialize(BinaryFile* fb) {
  ...
  if (_pThumb != nullptr) {
    ...
    if (false) {
      string_t cached = Gu::getPackage()->makeAssetPath("cache", getName() + "-thumb-saved.png");
      Gu::saveImage(cached, _pThumb);
    }
...
void ModelNode::drawForward(RenderParams& rp) {
  ...
  if (Gu::getRenderSettings()->getDebug()->getShowBoneBindBoxes()) {
    Gu::getActiveContext()->setLineWidth(1.0f);
    std::shared_ptr<UtilMeshInline> mi = std::make_shared<UtilMeshInline>(Gu::getActiveContext());
    mi->setDefaultColor(vec4(1, 1, 0, 1));
    mi->begin(GL_LINES);
