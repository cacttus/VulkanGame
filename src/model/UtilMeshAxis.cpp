...
void UtilMeshAxis::generate() {
  ...
  getVerts()->v3f(0) = (_mTransform * vec4(0, 0, 0, 1)).xyz();
  getVerts()->v3f(1) = (_mTransform * vec4(s, 0, 0, 1)).xyz();
  getVerts()->v3f(2) = (_mTransform * vec4(0, 0, 0, 1)).xyz();
  getVerts()->v3f(3) = (_mTransform * vec4(0, s, 0, 1)).xyz();
  getVerts()->v3f(4) = (_mTransform * vec4(0, 0, 0, 1)).xyz();
  getVerts()->v3f(5) = (_mTransform * vec4(0, 0, s, 1)).xyz();
