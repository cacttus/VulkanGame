...
public:
  ...
  BR2_FORCE_INLINE float getHitTime() { return _fHitTime; }
  BR2_FORCE_INLINE bool getIsContained() { return _bContained; }
...
public:
  ...
  BR2_FORCE_INLINE static bool isEven(int32_t i) {
    return ((i % 2) == 0);
  }
  BR2_FORCE_INLINE static bool isOdd(int32_t i) {
    return ((i % 2) != 0);
  }
  BR2_FORCE_INLINE static double brSqrt(double x) {
    return std::sqrt(x);
  }
  BR2_FORCE_INLINE static float brSqrt(float x) {
    //sqrtf undefined in clang..
    return (float)std::sqrt((double)x);
  }
  BR2_FORCE_INLINE static float brRsqrt(float x) {
    return 1.0f / brSqrt(x);
  }
  BR2_FORCE_INLINE static double brRsqrt(double x) {
    return 1.0 / std::sqrt(x);
  }
  BR2_FORCE_INLINE static float sqrf(float x) {
    return x * x;
  }
  static constexpr double radians(double d) {
    return (double)((d * (double)(M_PI)) / (double)180.0);
  }
  BR2_FORCE_INLINE static float degrees(float d) {
  ...
  BR2_FORCE_INLINE static float round(float in) {
    return (in >= 0.5000f) ? (ceilf(in)) : (floorf(in));
  }
  BR2_FORCE_INLINE static double round(double in) {
    return (in >= 0.5000) ? (ceil(in)) : (floor(in));
  }
  BR2_FORCE_INLINE static float round(float value, int precision) {
  ...
  BR2_FORCE_INLINE static double round(double value, int precision) {
    //TODO: std::round()
    const int adjustment = (int)pow(10, precision);
  ...
  template <class T>
  BR2_FORCE_INLINE static const T& brMin(const T& a, const T& b) {
    return a < b ? a : b;
  }
  template <class T>
  BR2_FORCE_INLINE static const T& brMax(const T& a, const T& b) {
    return a > b ? a : b;
  }
  template <class T>
  BR2_FORCE_INLINE static T& brMin(T& a, T& b) {
    return a < b ? a : b;
  }
  template <class T>
  BR2_FORCE_INLINE static T& brMax(T& a, T& b) {
    return a > b ? a : b;
  }
  template <class T>
  BR2_FORCE_INLINE static T& brClamp(T& x, T& a, T& b) {
    return brMin(b, brMax(x, a));
  }
  template <class T>
  BR2_FORCE_INLINE static const T& brClamp(const T& x, const T& a, const T& b) {
    return brMin(b, brMax(x, a));
  }
#endif
  BR2_FORCE_INLINE static float brSign(const float& val) {
    return (float)((float)(0.0f < val) - (float)(val < 0.0f));
  }
  //#define clamp(x,_min,_max) (broMin((_max),broMax((x),(_min))))
  BR2_FORCE_INLINE static bool fuzzyEquals(float fa, float fb, float epsilon = FUZZY_EQUALS_EPSILON) {
    float bm = fb - epsilon;
    float bp = fb + epsilon;
    return (fa > bm) && (fa < bp);
  }
  template <class T>
  BR2_FORCE_INLINE static const bool brInRangeInclusive(const T& x, const T& a, const T& b) {
    return ((x >= a) && (x <= b));
  }
  template <class T>
  BR2_FORCE_INLINE static const bool brInRangeInclusive(const T& x, const T& a) {
    return brInRangeInclusive(x, -a, a);
  }
  BR2_FORCE_INLINE static float modulateGridValue(float f_value, float f_grid_unit, float f_1_grid_unit) {
  ...
  BR2_FORCE_INLINE static int signi(float x) {
    //    Return -1, or 1 sign of x
    int a = (int)(x >= 0);
    return (a * (1)) + ((!a) * (-1));
  }
  BR2_FORCE_INLINE static float signf(float fx) {
    return (float)(signi(fx));
  }
  BR2_FORCE_INLINE static float ceil_signed(float x) {
