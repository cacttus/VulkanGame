...
public:
  ...
  STATIC Matrix3x3 getRotationRad(t_radians a, float x, float y, float z);    // - Returns a rotation matrix in radians.
  STATIC Matrix3x3 getRotationRad(float a, const vec3& v);    // - Returns a rotation matrix in radians.
...
FORCE_INLINE Matrix3x3::Matrix3x3(float(&f)[9]) {
  std::memcpy((float*)(&_m11), f, sizeof(float) * 9);
}
