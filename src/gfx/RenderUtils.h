...
public:
  ...
  static void drawWireSphereShader(std::shared_ptr<CameraNode> cam, float fRadius, const vec4& vColor, int32_t nSlices = 5, int32_t nStacks = 5, mat4* pMatrix = nullptr);
  static void drawWireBoxShader(std::shared_ptr<CameraNode> cam, const Box3f* box, const vec3& vOffset, const vec4& vColor);
  static void drawSolidBoxShaded(std::shared_ptr<CameraNode> cam, const Box3f* box, const vec3& vOffset, const vec4& vColor);
  //static void drawPickBox(Box3f* box, uint32_t uiColorId);
  static void drawAxisShader(std::shared_ptr<CameraNode> cam, float scale = 10.0f, float lineWidth = 2.0f, const mat4& transform = mat4::identity());  // Renders an Axis at the origin.
  static void drawFrustumShader(std::shared_ptr<CameraNode> cam, std::shared_ptr<FrustumBase> pf, const vec4& avColor);                                //idk why there are 2
