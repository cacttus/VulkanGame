...
public:
  ...
  static bool handleErrors(std::shared_ptr<GLContext> ctx, bool bShowNote, bool bDoNotBreak, bool doNotLog) {
  ...
  static bool checkOglErr(std::shared_ptr<GLContext> ctx, bool bShowNote, bool bDoNotBreak, bool doNotLog) {
    bool bError = false;
...
bool OglErr::chkErrRt(std::shared_ptr<GLContext> ctx, bool bDoNotBreak, bool doNotLog) {
  if (Gu::getEngineConfig()->getEnableRuntimeErrorChecking() == true) {
    return OglErr_Internal::handleErrors(ctx, true, bDoNotBreak, doNotLog);
  }
  return false;
}
