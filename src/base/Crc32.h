...
class Crc32 : public VirtualMemory {
 ...
 private:
  ...
  void initializeTable(uint32_t polynomial, uint32_t* createTable,
                       uint32_t tableLen = 256);
  uint32_t calculateHash(uint32_t* table, uint32_t seed, const char* buffer,
                         int start, int size);
};
