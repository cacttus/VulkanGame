...
template <typename Tx> void Allocator<Tx>::zeroMemory() {
  if (isAllocated()) {
...
void Allocator<Tx>::appendToEnd(const Allocator<Tx> *in) {
  ...
  //**Note: there was a problem here where we kept trying to append the same
  //buffer to
...
void Allocator<Tx>::appendToBeg(const Allocator<Tx> *in) {
  ...
  size_t oldInCount =
      in->count();  // must cache if we're copying a buffer to itself
 ...
 */
template <typename Tx> Tx *Allocator<Tx>::ptrOff(size_t offset) {
 ...
 */
template <typename Tx> void Allocator<Tx>::copyFrom(const Allocator<Tx> *in) {
 ...
 */
template <typename Tx>
void Allocator<Tx>::copyFrom(const Tx *other_in, size_t in_item_count,
                             size_t my_off, size_t other_off) {
 ...
 */
template <typename Tx> bool Allocator<Tx>::operator==(Tx *in_ptr) {
  return this->ptr() == in_ptr;
}
template <typename Tx> bool Allocator<Tx>::operator!=(Tx *in_ptr) {
  return this->ptr() != in_ptr;
}
template <typename Tx> bool Allocator<Tx>::operator==(Allocator<Tx> &in_ptr) {
 ...
 *    @fn at()
 */
template <typename Tx> Tx &Allocator<Tx>::at(size_t index) {
  //#if _DEBUG
...
template <typename Tx> void Allocator<Tx>::realloca(size_t newCount) {
  ...
  _pT = (Tx *)realloc(
      _pT,
      _allocSize);  // static_cast<Tx*>(GameMemoryManager::reallocBlock(_pT,
                    // _allocSize));
}
/**
 *    @fn dealloc()
 ...
 */
template <typename Tx> void Allocator<Tx>::dealloc() {
  if (_pT != NULL) {
    free(_pT);
  }
...
template <typename Tx> void Allocator<Tx>::reallocBytes(size_t newSize) {
  ...
  _pT = (Tx *)realloc(
      _pT,
      _allocSize);  // static_cast<Tx*>(MemoryManager::getSingleton()->mm_realloc(_pT,
                    // _allocSize));
}
/**
 *    @fn grow()
 ...
 *        @param itemOffset (in Tx Items) defaults to the beginning of the
 * buffer.
 ...
 *    have to reallocate the buffer to do this.  The buffer must be allocated
 * already to hold the new number of elements or an exception will be thrown.
 *    @note This does not alter the count() property.
 */
template <typename Tx> void Allocator<Tx>::inflate(
    size_t expandCount, size_t itemOffset,
    size_t staticAllocationCount  // The count that you are using in the buffer.
                                  // This is not the actual buffer size.
) {
  // Allocator<Tx> newData;
 ...
 *   ESSENTIALLY this SHIFTS the items from ITEM OFFSET to the ALLOOCATION COUNT
 * down to the DEFLATE COUNT move items at [start+count, allocation_count] to
 * (start) It is assumed the buffer can hold the given number of elements.
 ...
 */
template <typename Tx> void Allocator<Tx>::deflate(
    size_t deflateCount, size_t itemOffset,
    size_t staticAllocationCount  // The count that you are using in the buffer.
                                  // This is not the actual buffer size.
) {
  // Allocator<Tx> newData;
...
/**
 *    @fn erase()
 *    @brief Shifts elements from start+count down to start erasing any data in
 * between.
 *   @param count - number to erase after start
 */
template <typename Tx> void Allocator<Tx>::erase(size_t start, size_t count) {
...
template <typename Tx> void Allocator<Tx>::splice(size_t iAt, size_t iCount) {
  //    if((iAt + iCount > count()) || (!count()) || (!iCount))
  //        throw std::exception("Buffer over-write, size failure, or invalid
  //        param, in ManagedBuffer::splice.");
  //
  //    Tx* oldPtr, *newPtr;
  //    size_t newCount;
  //    size_t newSize;
  ...
  //        throw 0;//BroThrowException("[MB] Tried to allocate negative
  //        elements.",__LINE__,__FILE__);
  //
  //    if(iAt + iCount > count())
  //        throw 0;//BroThrowException("[MB] Tried to splice elements beyond
  //        the range of the buffer.",__LINE__,__FILE__);
  //
  //    _lastNum = _num;
  //    _lastSize = size();
  //
  ...
  //
  //        AssertOrThrow2((int)nCount - (iAt + iCount) >= 0);    //make sure
  //        we're not negative
  //
  ...
  //
  //        //Note - 2m version of safe_copy must be passed in valid block
  //        pointers. GameMemoryManager::safeCopy2m(newPtr, 0, _pT, offA, sizA);
 ...
 */
template <typename Tx> void Allocator<Tx>::insert(size_t iAt, const Tx &pData) {
  //    Tx* oldPtr, *newPtr;
  //    size_t newCount;
  //    size_t newSize;
  //    size_t _count = 1;
  ...
  //
  //    //if(_at+_count > size())
  //    //    throw new Exception("[MB] Tried to splice elements beyond the
  //    range of the buffer.",__LINE__,__FILE__);
  //    //
  //    // - New Params
  ...
  //            throw 0;//throw new Exception("[MB] Tried to allocate negative
  //            elements.", __LINE__, __FILE__);
  //
  //        size_t sizA = sizeof(Tx)*segACount;
  //        size_t sizB = sizeof(Tx)*segBCount;
 ...
 */
template <typename Tx>
void Allocator<Tx>::swapChunk(size_t iOff, size_t iCountToRemove,
