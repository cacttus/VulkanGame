...
class DateTime : public VirtualMemory {
  int _seconds; /* Seconds.	[0-60] (1 leap second) */
  int _minutes; /* Minutes.	[0-59] */
  int _hours;   /* Hours.	[0-23] */
  int _day;     /* Day.		[1-31] */
...
public:
  ...
  string_t toString();
  string_t dateToStr(const string_t& delim);
  string_t timeToStr(const string_t& delim, bool hour = true, bool minute = true, bool second = true, bool millisecond = true);
