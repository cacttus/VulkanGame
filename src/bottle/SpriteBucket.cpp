...
void SpriteBucket::loadMotions(std::shared_ptr<ObFile> obFile) {
  BRLogInfo("Loading Sprites and Motions.");
  ...
  for (; itSpec != _mapSpecs.end(); itSpec++) {
    ...
    //            if(sp->getMaskSpec() == nullptr){
    //                BRThrowException("Base tile ",sp->getName()," did not have a mask.  Please specify a mask ID for the base tile.");
    //            }
    //        }
    //    }
  }
  ...
  if (_pBlank == nullptr) {
    BRThrowException("Blank tile was not specified.  Please specify the name as 'tx-blank' to indicate the blank tile.");
  }
...
string_t SpriteSpec::getFirstFrameFileName() {
