...
std::shared_ptr<MeshSpec> MeshUtils::makeCrossboardXY(float xscale, float yscale, bool translateYToBottomOfBillboard) {
  ...
  mat4 m = mat4::rotation(Math::radians(90), 0, 1, 0);
...
std::shared_ptr<MeshNode> MeshUtils::createScreenQuadMesh(int w, int h) {
  ...
  quadMesh = MeshNode::create("ScreenQuadMesh_Node", false, std::make_shared<MeshSpec>("ScreenQuadMesh", verts.data(), verts.size(), inds.data(), inds.size(), v_v3x2::getVertexFormat(), nullptr));
