...
public:
  ...
  // virtual bool updateTouch(std::shared_ptr<InputManager> pFingers) override;
  void gameModeChanged(GameMode eMode);


  void setInputCaptured(bool b) { _bInputCaptured = b; }
private:
  int _iHoverVertex = 0;
  //GameMode _eGameMode = GameMode::Play; //Duplicate data - this is set in room.
