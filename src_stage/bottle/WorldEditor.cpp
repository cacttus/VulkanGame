...
void WorldEditor::init() {
  /*_pHoverShader = Gu::getContext()->getShaderMaker()->makeShader(
      std::vector<string_t>{"d_v3n3_hover.vs", "d_v3n3_hover.gs", "d_v3n3_hover.ps"}
  );*/
...
void WorldEditor::editWorldUpdateGrid(WorldCell* pc, std::shared_ptr<WorldGrid> pg, bool bMakeMeshRightAway,
  ...
  if (pg == nullptr && pc == nullptr) {
    BRThrowException("Error: both cell and grid can't be null.");
  }
  else if (pg != nullptr && pc != nullptr) {
    BRThrowException("Error: both cell and grid can't be set. ");
  }
...
void WorldEditor::editWorldEndEdit() {
  if (_bBeginEdit == true) {
    ...
    BRLogInfo("Saved " + _setGridsToSave.size() + " grids.");
...
void WorldEditor::editWorldMouseDown(std::shared_ptr<InputManager> pFingers) {
  ButtonState::e eLmb = pFingers->getLmbState();
  ButtonState::e eRmb = pFingers->getRmbState();
  EditTool::e eEditTool = _pWorldEditState->getEditTool();
  EditMode::e eEditMode = _pWorldEditState->getEditMode();
  ...
  if (eEditMode == EditMode::e::Tile) {
    ...
    if (pc != nullptr && (ms != W25TILE_EMPTY || eEditTool == EditTool::e::Eraser) && pc != getLastEditedCell()) {
      ...
      //     BRLogWarn("cell not found or motion not selected.");
    }
  }
  else if (eEditMode == EditMode::e::Block) {
...
void WorldEditor::playEditSound(GridMeshLayer::e eMatterMode, bool bDelete) {
  if (bDelete) {
    Gu::getSound()->tryPlay(Gu::getPackage()->makeAssetPath("sounds", "s-delete.ogg"));
  }
  else if (eMatterMode == GridMeshLayer::e::Opaque) {
    Gu::getSound()->tryPlay(Gu::getPackage()->makeAssetPath("sounds", "s-block-low.ogg"));
  }
  else if (eMatterMode == GridMeshLayer::e::Transparent) {
    Gu::getSound()->tryPlay(Gu::getPackage()->makeAssetPath("sounds", "s-water-splash.ogg"));
  }
  //    }
  //    else if (eEditMode == EditMode::e::Vertex || eEditMode == EditMode::e::VertexGroup) {
  //        if (eMatterMode == GridMeshLayer::e::Opaque) {
  //            Gu::getContext()->getSoundCache()->tryPlay(Gu::getPackage()->makeAssetPath("snd", "s-click.ogg"));
  //        }
  //        else if (eMatterMode == GridMeshLayer::e::Transparent) {
  //            Gu::getContext()->getSoundCache()->tryPlay(Gu::getPackage()->makeAssetPath("snd", "s-water-splash.ogg"));
  //        }
  //    }
  //}
  //else if (eEditTool == EditTool::e::Eraser) {
  //    Gu::getContext()->getSoundCache()->tryPlay(Gu::getPackage()->makeAssetPath("snd", "s-delete.ogg"));
  //}
}
void WorldEditor::editWorldMouseRelease(std::shared_ptr<InputManager> pFingers) {
  editWorldEndEdit();
...
bool WorldEditor::updateShiftEdit(std::shared_ptr<InputManager> pFingers, WorldCell* pc, vec3& n_out) {
  if (pFingers == nullptr) {
    return false;
  }
  if (pc == nullptr) {
...
void WorldEditor::addHistory(WorldCell* pc, int iPreviousGeom, int iPreviousTile, GridMeshLayer::e eMatterMode) {
  ...
  if (pc != nullptr) {
    if (_vecEditHistory.size() > 98) {
      ...
      BRLogWarn("Edit history exceeded maximum of 99.  Some history i8s lost");
    }
...
void WorldEditor::placeObject(std::shared_ptr<ModelSpec> ms) {
  if (_pPlacedObject == nullptr) {
    //TODO: reimplement the correct makeObj()
    BRThrowNotImplementedException();
    //_pPlacedObject = _pWorld25->makeObj(ms, vec3(0, 0, 0), vec4(0, 0, 1, 0), vec3(1, 1, 1), "");
